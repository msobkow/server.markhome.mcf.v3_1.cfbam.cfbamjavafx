// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash224Type.
...
/**
 *	CFBamJavaFXDbKeyHash224TypeAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash224Type.
 */
public class CFBamJavaFXDbKeyHash224TypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash224TypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash224TypeObj cur = getJavaFXFocusAsDbKeyHash224Type();
				if( cur != null ) {
					ICFBamDbKeyHash224TypeEditObj editObj = (ICFBamDbKeyHash224TypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash224TypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash224TypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash224TypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash224Type( argFocus );
	...
	public ICFBamDbKeyHash224TypeObj getEffJavaFXFocus() {
		ICFBamDbKeyHash224TypeObj eff = (ICFBamDbKeyHash224TypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash224TypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash224TypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash224TypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash224TypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash224TypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash224TypeObj)editObj.create();
