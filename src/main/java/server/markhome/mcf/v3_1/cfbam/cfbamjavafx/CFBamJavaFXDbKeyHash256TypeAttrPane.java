// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash256Type.
...
/**
 *	CFBamJavaFXDbKeyHash256TypeAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash256Type.
 */
public class CFBamJavaFXDbKeyHash256TypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash256TypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash256TypeObj cur = getJavaFXFocusAsDbKeyHash256Type();
				if( cur != null ) {
					ICFBamDbKeyHash256TypeEditObj editObj = (ICFBamDbKeyHash256TypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash256TypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash256TypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash256TypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash256Type( argFocus );
	...
	public ICFBamDbKeyHash256TypeObj getEffJavaFXFocus() {
		ICFBamDbKeyHash256TypeObj eff = (ICFBamDbKeyHash256TypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash256TypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash256TypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash256TypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash256TypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash256TypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash256TypeObj)editObj.create();
