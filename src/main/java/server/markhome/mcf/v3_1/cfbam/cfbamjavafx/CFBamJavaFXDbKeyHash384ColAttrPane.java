// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash384Col.
...
/**
 *	CFBamJavaFXDbKeyHash384ColAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash384Col.
 */
public class CFBamJavaFXDbKeyHash384ColAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash384ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash384ColObj cur = getJavaFXFocusAsDbKeyHash384Col();
				if( cur != null ) {
					ICFBamDbKeyHash384ColEditObj editObj = (ICFBamDbKeyHash384ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash384ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash384ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash384ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash384Col( argFocus );
	...
	public ICFBamDbKeyHash384ColObj getEffJavaFXFocus() {
		ICFBamDbKeyHash384ColObj eff = (ICFBamDbKeyHash384ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash384ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash384ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash384ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash384ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash384ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash384ColObj)editObj.create();
