// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash128Def.
...
/**
 *	CFBamJavaFXDbKeyHash128DefAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash128Def.
 */
public class CFBamJavaFXDbKeyHash128DefAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash128DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash128DefObj cur = getJavaFXFocusAsDbKeyHash128Def();
				if( cur != null ) {
					ICFBamDbKeyHash128DefEditObj editObj = (ICFBamDbKeyHash128DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash128DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash128DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash128DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash128Def( argFocus );
	...
	public ICFBamDbKeyHash128DefObj getEffJavaFXFocus() {
		ICFBamDbKeyHash128DefObj eff = (ICFBamDbKeyHash128DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash128DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash128DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash128DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash128DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash128DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash128DefObj)editObj.create();
