// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash512Col.
...
/**
 *	CFBamJavaFXDbKeyHash512ColAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash512Col.
 */
public class CFBamJavaFXDbKeyHash512ColAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash512ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash512ColObj cur = getJavaFXFocusAsDbKeyHash512Col();
				if( cur != null ) {
					ICFBamDbKeyHash512ColEditObj editObj = (ICFBamDbKeyHash512ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash512ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash512ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash512ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash512Col( argFocus );
	...
	public ICFBamDbKeyHash512ColObj getEffJavaFXFocus() {
		ICFBamDbKeyHash512ColObj eff = (ICFBamDbKeyHash512ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash512ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash512ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash512ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash512ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash512ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash512ColObj)editObj.create();
