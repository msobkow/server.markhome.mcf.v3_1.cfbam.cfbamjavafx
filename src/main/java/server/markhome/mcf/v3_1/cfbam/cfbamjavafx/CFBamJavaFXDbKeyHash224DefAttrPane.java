// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash224Def.
...
/**
 *	CFBamJavaFXDbKeyHash224DefAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash224Def.
 */
public class CFBamJavaFXDbKeyHash224DefAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash224DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash224DefObj cur = getJavaFXFocusAsDbKeyHash224Def();
				if( cur != null ) {
					ICFBamDbKeyHash224DefEditObj editObj = (ICFBamDbKeyHash224DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash224DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash224DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash224DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash224Def( argFocus );
	...
	public ICFBamDbKeyHash224DefObj getEffJavaFXFocus() {
		ICFBamDbKeyHash224DefObj eff = (ICFBamDbKeyHash224DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash224DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash224DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash224DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash224DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash224DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash224DefObj)editObj.create();
