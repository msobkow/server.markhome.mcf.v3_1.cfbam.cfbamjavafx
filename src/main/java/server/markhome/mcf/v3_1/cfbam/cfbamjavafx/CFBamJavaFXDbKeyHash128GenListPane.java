// Description: Java 25 JavaFX List of Obj Pane implementation for DbKeyHash128Gen.
...
/**
 *	CFBamJavaFXDbKeyHash128GenListPane JavaFX List of Obj Pane implementation
 *	for DbKeyHash128Gen.
 */
public class CFBamJavaFXDbKeyHash128GenListPane
extends CFBorderPane
implements ICFBamJavaFXDbKeyHash128GenPaneList
{
	public static String S_FormName = "List DbKeyHash128Gen";
	...
	public CFBamJavaFXDbKeyHash128GenListPane( ICFFormManager formManager,
		...
		boolean sortByChain )
	{
		...
		if( observableListOfDbKeyHash128Gen != null ) {
	...
	public CFHBox getPanelHBoxMenu() {
		if( hboxMenu == null ) {
			...
			hboxMenu.getChildren().add( buttonAddDbKeyHash128Gen );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							if( entry.getSchemaName().equals("CFBam") && backingClassCode == ICFBamDbKeyHash128Gen.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getDbKeyHash128GenFactory().newViewEditForm( cfFormManager, selectedInstance, getViewEditClosedCallback(), false );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							int backingClassCode = entry.getBackingClassCode();
							if( entry.getSchemaName().equals("CFBam") && backingClassCode == ICFBamDbKeyHash128Gen.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getDbKeyHash128GenFactory().newAskDeleteForm( cfFormManager, selectedInstance, getDeleteCallback() );
								((ICFBamJavaFXDbKeyHash128GenPaneCommon)frame).setPaneMode( CFPane.PaneMode.View );
								cfFormManager.pushForm( frame );
							}
							else {
	...
	public void adjustListButtons() {
		...
		if( buttonAddDbKeyHash128Gen != null ) {
			buttonAddDbKeyHash128Gen.setDisable( ! allowAdds );
		}
