// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash224Col.
...
/**
 *	CFBamJavaFXDbKeyHash224ColAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash224Col.
 */
public class CFBamJavaFXDbKeyHash224ColAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash224ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash224ColObj cur = getJavaFXFocusAsDbKeyHash224Col();
				if( cur != null ) {
					ICFBamDbKeyHash224ColEditObj editObj = (ICFBamDbKeyHash224ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash224ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash224ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash224ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash224Col( argFocus );
	...
	public ICFBamDbKeyHash224ColObj getEffJavaFXFocus() {
		ICFBamDbKeyHash224ColObj eff = (ICFBamDbKeyHash224ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash224ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash224ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash224ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash224ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash224ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash224ColObj)editObj.create();
