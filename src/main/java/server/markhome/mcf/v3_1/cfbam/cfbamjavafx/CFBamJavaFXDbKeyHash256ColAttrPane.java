// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash256Col.
...
/**
 *	CFBamJavaFXDbKeyHash256ColAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash256Col.
 */
public class CFBamJavaFXDbKeyHash256ColAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash256ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash256ColObj cur = getJavaFXFocusAsDbKeyHash256Col();
				if( cur != null ) {
					ICFBamDbKeyHash256ColEditObj editObj = (ICFBamDbKeyHash256ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash256ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash256ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash256ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash256Col( argFocus );
	...
	public ICFBamDbKeyHash256ColObj getEffJavaFXFocus() {
		ICFBamDbKeyHash256ColObj eff = (ICFBamDbKeyHash256ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash256ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash256ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash256ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash256ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash256ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash256ColObj)editObj.create();
