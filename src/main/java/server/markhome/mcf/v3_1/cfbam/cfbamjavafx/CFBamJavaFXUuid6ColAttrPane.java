// Description: Java 25 JavaFX Attribute Pane implementation for Uuid6Col.
...
/**
 *	CFBamJavaFXUuid6ColAttrPane JavaFX Attribute Pane implementation
 *	for Uuid6Col.
 */
public class CFBamJavaFXUuid6ColAttrPane
extends CFGridPane
implements ICFBamJavaFXUuid6ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuid6ColObj cur = getJavaFXFocusAsUuid6Col();
				if( cur != null ) {
					ICFBamUuid6ColEditObj editObj = (ICFBamUuid6ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuid6ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUuid6ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuid6ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsUuid6Col( argFocus );
	...
	public ICFBamUuid6ColObj getEffJavaFXFocus() {
		ICFBamUuid6ColObj eff = (ICFBamUuid6ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuid6ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuid6ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamUuid6ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUuid6ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuid6ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuid6ColObj)editObj.create();
