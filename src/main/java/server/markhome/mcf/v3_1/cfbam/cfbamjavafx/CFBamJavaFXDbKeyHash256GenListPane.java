// Description: Java 25 JavaFX List of Obj Pane implementation for DbKeyHash256Gen.
...
/**
 *	CFBamJavaFXDbKeyHash256GenListPane JavaFX List of Obj Pane implementation
 *	for DbKeyHash256Gen.
 */
public class CFBamJavaFXDbKeyHash256GenListPane
extends CFBorderPane
implements ICFBamJavaFXDbKeyHash256GenPaneList
{
	public static String S_FormName = "List DbKeyHash256Gen";
	...
	public CFBamJavaFXDbKeyHash256GenListPane( ICFFormManager formManager,
		...
		boolean sortByChain )
	{
		...
		if( observableListOfDbKeyHash256Gen != null ) {
	...
	public CFHBox getPanelHBoxMenu() {
		if( hboxMenu == null ) {
			...
			hboxMenu.getChildren().add( buttonAddDbKeyHash256Gen );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							if( entry.getSchemaName().equals("CFBam") && backingClassCode == ICFBamDbKeyHash256Gen.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getDbKeyHash256GenFactory().newViewEditForm( cfFormManager, selectedInstance, getViewEditClosedCallback(), false );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							int backingClassCode = entry.getBackingClassCode();
							if( entry.getSchemaName().equals("CFBam") && backingClassCode == ICFBamDbKeyHash256Gen.CLASS_CODE ) {
								CFBorderPane frame = javafxSchema.getDbKeyHash256GenFactory().newAskDeleteForm( cfFormManager, selectedInstance, getDeleteCallback() );
								((ICFBamJavaFXDbKeyHash256GenPaneCommon)frame).setPaneMode( CFPane.PaneMode.View );
								cfFormManager.pushForm( frame );
							}
							else {
	...
	public void adjustListButtons() {
		...
		if( buttonAddDbKeyHash256Gen != null ) {
			buttonAddDbKeyHash256Gen.setDisable( ! allowAdds );
		}
