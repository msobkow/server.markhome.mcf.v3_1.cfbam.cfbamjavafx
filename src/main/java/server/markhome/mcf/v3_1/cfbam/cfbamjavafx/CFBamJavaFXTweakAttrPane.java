// Description: Java 25 JavaFX Attribute Pane implementation for Tweak.
...
/**
 *	CFBamJavaFXTweakAttrPane JavaFX Attribute Pane implementation
 *	for Tweak.
 */
public class CFBamJavaFXTweakAttrPane
extends CFGridPane
implements ICFBamJavaFXTweakPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTweakObj cur = getJavaFXFocusAsTweak();
				if( cur != null ) {
					ICFBamTweakEditObj editObj = (ICFBamTweakEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTweakObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTweakAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTweakObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsTweak( argFocus );
	...
	public ICFBamTweakObj getEffJavaFXFocus() {
		ICFBamTweakObj eff = (ICFBamTweakObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTweakObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTweakObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTweakObj focus = getJavaFXFocusAsTweak();
		ICFBamTweakEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTweakEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTweakEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTweakObj)editObj.create();
