// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash128Type.
...
/**
 *	CFBamJavaFXDbKeyHash128TypeAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash128Type.
 */
public class CFBamJavaFXDbKeyHash128TypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash128TypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash128TypeObj cur = getJavaFXFocusAsDbKeyHash128Type();
				if( cur != null ) {
					ICFBamDbKeyHash128TypeEditObj editObj = (ICFBamDbKeyHash128TypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash128TypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash128TypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash128TypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash128Type( argFocus );
	...
	public ICFBamDbKeyHash128TypeObj getEffJavaFXFocus() {
		ICFBamDbKeyHash128TypeObj eff = (ICFBamDbKeyHash128TypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash128TypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash128TypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash128TypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash128TypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash128TypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash128TypeObj)editObj.create();
