// Description: Java 25 JavaFX Attribute Pane implementation for Uuid6Type.
...
/**
 *	CFBamJavaFXUuid6TypeAttrPane JavaFX Attribute Pane implementation
 *	for Uuid6Type.
 */
public class CFBamJavaFXUuid6TypeAttrPane
extends CFGridPane
implements ICFBamJavaFXUuid6TypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuid6TypeObj cur = getJavaFXFocusAsUuid6Type();
				if( cur != null ) {
					ICFBamUuid6TypeEditObj editObj = (ICFBamUuid6TypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuid6TypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUuid6TypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuid6TypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsUuid6Type( argFocus );
	...
	public ICFBamUuid6TypeObj getEffJavaFXFocus() {
		ICFBamUuid6TypeObj eff = (ICFBamUuid6TypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuid6TypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuid6TypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamUuid6TypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUuid6TypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuid6TypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuid6TypeObj)editObj.create();
