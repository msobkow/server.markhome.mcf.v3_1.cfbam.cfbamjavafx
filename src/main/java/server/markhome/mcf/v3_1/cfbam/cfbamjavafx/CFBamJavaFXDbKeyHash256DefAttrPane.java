// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash256Def.
...
/**
 *	CFBamJavaFXDbKeyHash256DefAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash256Def.
 */
public class CFBamJavaFXDbKeyHash256DefAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash256DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash256DefObj cur = getJavaFXFocusAsDbKeyHash256Def();
				if( cur != null ) {
					ICFBamDbKeyHash256DefEditObj editObj = (ICFBamDbKeyHash256DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash256DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash256DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash256DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash256Def( argFocus );
	...
	public ICFBamDbKeyHash256DefObj getEffJavaFXFocus() {
		ICFBamDbKeyHash256DefObj eff = (ICFBamDbKeyHash256DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash256DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash256DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash256DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash256DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash256DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash256DefObj)editObj.create();
