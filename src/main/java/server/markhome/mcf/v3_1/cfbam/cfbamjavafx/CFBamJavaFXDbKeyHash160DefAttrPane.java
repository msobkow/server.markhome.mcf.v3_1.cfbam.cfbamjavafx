// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash160Def.
...
/**
 *	CFBamJavaFXDbKeyHash160DefAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash160Def.
 */
public class CFBamJavaFXDbKeyHash160DefAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash160DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash160DefObj cur = getJavaFXFocusAsDbKeyHash160Def();
				if( cur != null ) {
					ICFBamDbKeyHash160DefEditObj editObj = (ICFBamDbKeyHash160DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash160DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash160DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash160DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamDbKeyHash160DefObj getEffJavaFXFocus() {
		ICFBamDbKeyHash160DefObj eff = (ICFBamDbKeyHash160DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash160DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash160DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash160DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash160DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash160DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash160DefObj)editObj.create();
