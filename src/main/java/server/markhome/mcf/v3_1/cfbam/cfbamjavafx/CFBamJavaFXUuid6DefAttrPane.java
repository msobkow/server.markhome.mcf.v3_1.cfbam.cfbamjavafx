// Description: Java 25 JavaFX Attribute Pane implementation for Uuid6Def.
...
/**
 *	CFBamJavaFXUuid6DefAttrPane JavaFX Attribute Pane implementation
 *	for Uuid6Def.
 */
public class CFBamJavaFXUuid6DefAttrPane
extends CFGridPane
implements ICFBamJavaFXUuid6DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuid6DefObj cur = getJavaFXFocusAsUuid6Def();
				if( cur != null ) {
					ICFBamUuid6DefEditObj editObj = (ICFBamUuid6DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuid6DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUuid6DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuid6DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsUuid6Def( argFocus );
	...
	public ICFBamUuid6DefObj getEffJavaFXFocus() {
		ICFBamUuid6DefObj eff = (ICFBamUuid6DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuid6DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuid6DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamUuid6DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUuid6DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuid6DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuid6DefObj)editObj.create();
