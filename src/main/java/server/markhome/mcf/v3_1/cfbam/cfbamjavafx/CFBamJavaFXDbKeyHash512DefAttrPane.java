// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash512Def.
...
/**
 *	CFBamJavaFXDbKeyHash512DefAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash512Def.
 */
public class CFBamJavaFXDbKeyHash512DefAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash512DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash512DefObj cur = getJavaFXFocusAsDbKeyHash512Def();
				if( cur != null ) {
					ICFBamDbKeyHash512DefEditObj editObj = (ICFBamDbKeyHash512DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash512DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash512DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash512DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDbKeyHash512Def( argFocus );
	...
	public ICFBamDbKeyHash512DefObj getEffJavaFXFocus() {
		ICFBamDbKeyHash512DefObj eff = (ICFBamDbKeyHash512DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash512DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash512DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash512DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash512DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash512DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash512DefObj)editObj.create();
