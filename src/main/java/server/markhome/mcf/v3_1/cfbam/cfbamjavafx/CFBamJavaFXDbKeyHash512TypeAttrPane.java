// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash512Type.
...
/**
 *	CFBamJavaFXDbKeyHash512TypeAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash512Type.
 */
public class CFBamJavaFXDbKeyHash512TypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash512TypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash512TypeObj cur = getJavaFXFocusAsDbKeyHash512Type();
				if( cur != null ) {
					ICFBamDbKeyHash512TypeEditObj editObj = (ICFBamDbKeyHash512TypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash512TypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash512TypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash512TypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamDbKeyHash512TypeObj getEffJavaFXFocus() {
		ICFBamDbKeyHash512TypeObj eff = (ICFBamDbKeyHash512TypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash512TypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash512TypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash512TypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash512TypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash512TypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash512TypeObj)editObj.create();
