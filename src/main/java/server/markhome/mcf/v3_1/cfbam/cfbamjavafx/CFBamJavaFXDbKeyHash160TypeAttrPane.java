// Description: Java 25 JavaFX Attribute Pane implementation for DbKeyHash160Type.
...
/**
 *	CFBamJavaFXDbKeyHash160TypeAttrPane JavaFX Attribute Pane implementation
 *	for DbKeyHash160Type.
 */
public class CFBamJavaFXDbKeyHash160TypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDbKeyHash160TypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDbKeyHash160TypeObj cur = getJavaFXFocusAsDbKeyHash160Type();
				if( cur != null ) {
					ICFBamDbKeyHash160TypeEditObj editObj = (ICFBamDbKeyHash160TypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDbKeyHash160TypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDbKeyHash160TypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDbKeyHash160TypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamDbKeyHash160TypeObj getEffJavaFXFocus() {
		ICFBamDbKeyHash160TypeObj eff = (ICFBamDbKeyHash160TypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDbKeyHash160TypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDbKeyHash160TypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDbKeyHash160TypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDbKeyHash160TypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDbKeyHash160TypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDbKeyHash160TypeObj)editObj.create();
